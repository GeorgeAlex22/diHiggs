...
void Selection()
{
  ...
  {  
    char buffer[100];
    int  file_index, nfiles = 2;

    for(file_index=1; file_index<nfiles+1; file_index++)
    {
    
      // sprintf(buffer, "delphes_output%d.root", file_index);
      if(file_index<10) {sprintf(buffer, "/eos/cms/store/group/phys_b2g/giorgos/pheno/rootFiles/pptohh/trees/run_0%d.lhe.hep.root", file_index);}
      else {sprintf(buffer, "/eos/cms/store/group/phys_b2g/giorgos/pheno/rootFiles/pptohh/trees/run_%d.lhe.hep.root", file_index);}
    
      chain->Add(buffer);

      treeResults  = new TFile("signalSelection.root", "recreate");
    }
  }
  ...
  else
  {
    ...
    chain->Add("/eos/cms/store/group/phys_b2g/giorgos/pheno/rootFiles/pptohh/trees/run_100.lhe.hep.root");
    //chain->Add("/tmp/giorgos/delphes_output.root");
  }
...
//////////////////////////////////////////////////////////////////
  ...
  if(signal_flag)
  {
    if(generatedInfo){treeResults  = new TFile("tree_pptohh_GEN.root", "recreate");}
    else {treeResults  = new TFile("tree_pptohh_RECO.root", "recreate");}  
  } 

  else {treeResults  = new TFile("tree.root", "recreate");}   
